...
class CloudFlareTest < ActiveSupport::TestCase
  ...
  test 'error responses' do
    ...
    assert_equal 'CloudFlare API error: HTTP response code 400, {"result"=>"success", "response"=>{"recs"=>{"objs"=>[]}}}', res
    ...
    assert_match /cloudflare api error: http response code 500, {/i, res
    ...
    assert_equal 'CloudFlare API error: response result not_sucess, {"result"=>"not_sucess", "response"=>{"recs"=>{"objs"=>[{"rec_id"=>1, "display_name"=>"abc", "type"=>"A"}]}}}', res
    ...
    res = @cloudflare.dns_update('abc', 'localhost')
    assert res.error?, 'CloudFlare response should have been result not success'
    assert_equal 'CloudFlare API error: response result not_success, {"result"=>"not_success", "response"=>{}}', res
    ...
    assert res.error?, 'CloudFlare response should have been result not success'
    assert_match /cloudflare api error: response result not_success/i, res
    ...
    res = @cloudflare.dns_delete('abc')
    assert res.error?, 'CloudFlare response should have been an error 400'
    assert_equal 'CloudFlare API error: HTTP response code 400, {"result"=>"success", "response"=>{}}', res
  end
  ...
  test 'http timeout' do
    ...
    assert_equal 'CloudFlare API network exception: execution expired', res
  end
end
